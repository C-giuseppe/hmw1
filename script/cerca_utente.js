      ...
      if(cerca_nome.classList.value !== 'onok'){
       ...
       // console.log(users_view);
        if(users_view.classList.value !== 'onok'){
            ...
            anime_view.classList.add('off');
            esposizione.classList.add('off');
            fetch("request_all_users.php").then(onResponse).then(onJson2);
        }else {
            users_view.classList.remove('onok');
            esposizione.classList.add('off');
            anime_view.classList.add('off');
...
function onJson(json){ 
    ...
    if ( json === "Nessun risultato" ){
        ...
        fetch("request_anime.php?username=" + username).then(onResponse).then(onJson1);
    }
    if(cerca_nome.classList.value !== 'onok'){
...
function cerca(event){ 
    ...
    fetch("request_search_users.php?cercausername="+cercausername).then(onResponse).then(onJson);
